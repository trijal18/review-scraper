...
def review_url(url):
    ...
    # Split path to get product ID (itm911b47c91c5dd)
    path_parts = parsed_url.path.split('/')
    product_id = path_parts[-1].split('?')[0]
    
    # Create the reviews page URL
    reviews_url = f"https://www.flipkart.com{parsed_url.path}/product-reviews/{product_id}"
    ...
    # Rebuild the query string with the updated page
    new_query_string = urlencode(query_params, doseq=True)
    
    # Construct the final reviews URL with the updated query string
    final_url = f"{reviews_url}?{new_query_string}"
...
for i in range(1, 44):
    ...
    url=review_url(url)+f"&marketplace=FLIPKART&page={i}"
